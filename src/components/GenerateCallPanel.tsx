...
import { ProcessingTimeline, type ProcessingStep } from './ProcessingTimeline'
import { useVapiCall } from '../context/VapiCallContext'
import { api, type GenerateStreamEvent } from '../lib/api'
...
export function GenerateCallPanel({ onGenerated, onClose }: GenerateCallPanelProps) {
  const { acceptanceProfile } = useVapiCall()
  ...
  function applyEvent(event: GenerateStreamEvent) {
    if (event.type === 'plan') {
      setSteps(event.steps.map((s) => ({ ...s, status: 'pending' as const })))
  ...
  async function handleGenerate() {
    ...
    try {
      await api.generateCallStream(
        ...
        (event) => {
          applyEvent(event)
          if (event.type === 'done') generatedId = event.call.id
          if (event.type === 'error') throw new Error(event.error)
        },
      )
      if (generatedId) {
        onGenerated(generatedId)
      } else {
  ...
  return (
    <div className="rounded-domu-lg bg-app-card border border-domu-blue/30 p-5 space-y-4 transition-colors">
      ...
      {steps.length > 0 && <ProcessingTimeline steps={steps} />}
